...
function recommendRank(username){
  newRanks = []
  pos = -1
  for (var i = 0; i < userRanks.length; i++) {
    if (userRanks[i]['User'] == username) pos = i
    else newRanks.push(userRanks[i])
  }

  valores = newRanks.sort(compareRanks)

  return (valores.slice(0,5))

}

function RecommendPond(username) {
  pos = -1
  for (var i = 0; i < userList.length; i++) {
    if (userList[i] == username) {
      pos = i
    ...
    if (pos != i) {
      if(userMatrix[pos][i] != 1) {
        ...
        for (var j = 0; j < userRanks.length; j++) {
          if (userRanks[j]['User'] == userMatrix[pos][i]){
            pos2 = j
          }
        }
        newScore = valor * userRanks[pos2]['Rank']
        var score = {user:userList[i], score:newScore};
        valores.push(score)
      }
    }
...
function recommendAll(username){
  return ([recommend(username), recommendRank(username), RecommendPond(username)])
}
...
function loadCsv(){
  ...
  d3.csv("https://raw.githubusercontent.com/ajgan/RC_LinkPrediction/master/my-app/src/assets/cluster2frame.csv", function(csv) {
    data = csv
...
function loadCsv2() {
  ...
  d3.csv("https://raw.githubusercontent.com/ajgan/RC_LinkPrediction/master/my-app/src/assets/pagerank72.csv", function(csv) {
    userRanks = []
    for (var i = 0; i < csv.length; i++) {
      name = csv[i]["Target"]
      rank = csv[i][]
      userRanks.push({User:name, Rank:rank})
    }
  });
